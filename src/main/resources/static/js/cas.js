...
function disableEmptyInputFormSubmission() {
    var fields = $('#fm1 input[name="username"],[name="password"],[name="captcha"]');

    if (fields.length == 3) {
        fields.on('input', function (event) {
            var enableSubmission = $('#fm1 input[name="username"]').val().trim() &&
                $('#fm1 input[name="password"]').val().trim() && $('#fm1 input[name="captcha"]').val().trim();

            if (enableSubmission && enableSubmission != '') {
